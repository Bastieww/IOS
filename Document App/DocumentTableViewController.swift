...
class DocumentTableViewController: UITableViewController, QLPreviewControllerDataSource, UIDocumentPickerDelegate {
    ...
    struct DocumentFile {
        ...
        // This static property is now redundant, since we're getting documents dynamically
    }
    ...
    func listFileInDocumentsDirectory() -> [DocumentFile] {
        ...
        // Adding imported documents (from Documents directory)
        for item in items {
            ...
            documentList.append(DocumentFile(
                ...
                url: item,
                type: "application/octet-stream",  // You can refine this based on file types
                source: .imported  // This is an imported file
            ))
        }
        
        // Adding bundled documents (from app bundle)
        documentList.append(contentsOf: listFileInBundle())  // Adding bundled documents
        
        return documentList
    }
    
    // MARK: - Get Bundle Images
    func listFileInBundle() -> [DocumentFile] {
        ...
        for item in items {
            if !item.hasSuffix("DS_Store") && (item.hasSuffix(".jpg") || (item.hasSuffix(".pdf")))  {
